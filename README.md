...
> **Статус:** клиент берёт локации и режимы мойки из API, история пока захардкожена на клиенте.
> Опубликованное демо на время осталось без точек: в образе Cloud Run лежит только статика, API рядом
> нет. Чинится следующей веткой — клиент и сервер собираются в один контейнер, база уезжает в Neon.
...
.github/workflows/        CI (сборка клиента + агент код-ревью) и CD (деплой в Cloud Run)
...
сервер и падает, если база уже существует: приводить существующую базу к нужному виду — работа миграций.
...
Прогон xUnit в пайплайн ещё не добавлен: тесты поднимают Postgres в Docker и пока запускаются локально.
...
- [ ] Клиент и API в одном контейнере, база в Neon — чиним опубликованное демо
