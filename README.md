...
База поднимается контейнером, схема и справочные данные приезжают одним скриптом:
...
```bash
docker exec -i robowash-db psql -U postgres < db/create-database.sql
```

Скрипт [db/create-database.sql](db/create-database.sql) создаёт базу `robowash`, все таблицы и справочные
данные — семь точек и три режима мойки, те же, что сейчас захардкожены на клиенте. Он рассчитан на чистый
сервер и упадёт, если база уже существует: приводить существующую базу к нужному виду — работа миграций.
