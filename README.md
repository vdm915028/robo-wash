...
- **На пуш в `main`** — деплой обоих сервисов. Сначала API: образ уезжает в Artifact Registry, сервис
  `robo-wash-api` обновляется и отдаёт наружу свой адрес. Следом клиент — он собирается уже с этим адресом
  и выкатывается в `robo-wash`. Адрес API нигде не дублируется, он приходит выходом предыдущего job'а.
